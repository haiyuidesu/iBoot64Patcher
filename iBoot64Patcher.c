        ...
        if ((op & 0xffc003ff) == 0x910003fd) {
            ...
            if ((delta & 0xf) == 0) {
                ...
                while (where > start) {
                    ...
                    if (((au & 0xffc003ff) == 0xd10003ff) && (((au >> 0xA) & 0xfff) == delta + 0x10)) {
                        return where;
                    }
...
uint64_t enable_kernel_debug(void *ibot, unsigned int length) {
    ...
    uint64_t insn = insn_is_bl(ibot, xref, pac_set(6723, 0x5, 0x2), 0x4);
...
uint64_t allow_any_imagetype(void *ibot, unsigned int length) {
    ...
    void *where = memmem(ibot, length, str, strlen(str)); // will search for the string into the file (at the end)

    if (where == NULL) return -1;

    uint64_t xref = xref64(ibot, 0x0, length, where - ibot); // will set itself at the string real position

    if (xref == 0) return -1;

    uint64_t insn = ((version < 3406) ? 0x040080d2 : 0x050080d2); // mov x4, #0 | mov x5, #0

    *(uint32_t *)(ibot + xref) = bswap32(insn); // will apply the patch at this address ('type' variable)

    insn = hex_set(5540, hex_set(3406, 0xe5071f32, 0xe60b0032), 0xe6008052);

    where = memdata(ibot, bswap32(insn), 0x4, ibot, length); // will search for a specific instruction
                                                             // (which is set in the line above)

    insn = (version < 3406) ? 0x05008052 : 0x06008052; // mov w5, #0 | mov w6, #0

    *(uint32_t *)(where) = bswap32(insn); // will apply another patch at this address ('count' variable)
...
uint64_t prevent_kaslr_slide(void *ibot, unsigned int length) {
    ...
    if (version < 3406) return 0;

    if (version > 4513) return 0; // only for iOS 10 to iOS 12 for the moment

    printf("\n[%s]: patching the kaslr slide...\n", __func__);

    void *current = memmem(ibot, length, "__PAGEZERO", sizeof("__PAGEZERO"));
    ...
    printf("[%s]: successfully patched the kaslr slide!\n", __func__);
...
int main(int argc, char *argv[]) {
    ...
    if (patch) {
        ...
        printf("[%s]: missing flag \"-p\", please execute iBoot64Patcher again.", __func__);
